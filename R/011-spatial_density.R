...
heatmap.m <- dplyr::filter(heatmap, in_area(lon, lat, area))
...
saveGIF({
  for ( hour in 7:23) {
    hm.hour <- dplyr::filter(heatmap.m, tod==hour)
    fit.hist(hm.hour$z, xlab="Population of each cell", ylab="Density", main=paste("Hour ", hour, sep=""))
...
for( hour in hours){
  ...
  hm.hour <- dplyr::filter(heatmap.m, tod==hour)
  fit.hist(hm.hour$z, xlab="Population of each cell", ylab="Density", main="")
  dev.off()
}
...
mln.hourly <- function(hm){
  do.call(rbind, lapply(7:23, FUN = function(hour){
    # filter hourly data
    z <- dplyr::filter(hm, tod==hour & z >0)[["z"]]
...
ks.hourly <- do.call(rbind, lapply(7:23, FUN = function(hour){
  print(sprintf("Processing hour %2d:00", hour))
  v <- dplyr::filter(heatmap.m, tod==hour & z>0)[["z"]]
...
plot.ks.hourly(dplyr::filter(ks.hourly, h>=7 & h <= 23))
dev.off()